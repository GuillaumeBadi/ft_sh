...
/*   Updated: 2015/01/03 19:23:09 by gbadi            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_get_command(char *command, char **path, char ***env, t_alias **alias)
{
	...
	if (ft_strlen(ft_strtrim(command)) == 0)
		...
		ft_clear();
	else if (ft_strnequ(command, "last", 4))
	...
	path = path;
	env = env;
	if (!ft_strnequ(command, "alias", 5) && !ft_strnequ(command, "last", 4) && ret == 0)
		ft_putline("minishell-history", command);
	return (ret);
}
